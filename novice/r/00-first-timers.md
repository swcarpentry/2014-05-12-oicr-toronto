...
As such R is extremely useful both for statistics and data science.
...
`install.packages("package-name")` will download a package from one of the CRAN mirrors.
...
hours <- 24
days <- 365
secondsPerYear <- seconds * minutes * hours * days
```
...
Converting "hello" to a numeric vector yielded a warning that NAs were introduced by coercion. NAs represent missing data in R.
